...
QUESTIONS = [
    {
        ...
        'type': 'select',
        'options': None,  # заполняется в render_predict из bundle
    ...
    {
        'key': 'ppltrst',
        'label': 'Большинству людей можно доверять?',
        'type': 'slider', 'min': 0, 'max': 10, 'step': 1,
        'min_label': 'Нельзя доверять', 'max_label': 'Можно доверять',
    },
    {
        'key': 'pplhlp',
        'label': 'Большинство людей готовы помочь другим?',
        'type': 'slider', 'min': 0, 'max': 10, 'step': 1,
        'min_label': 'Думают только о себе', 'max_label': 'Готовы помочь',
    },
    {
        'key': 'happy',
...
def render_predict(bundle):
    ...
    QUESTIONS[0]['options'] = sorted(COUNTRY_NAMES.get(c, c) for c in countries)
        ...
        if wkey not in st.session_state:
            if q['key'] in st.session_state.answers:
                ...
                st.session_state[wkey] = (
                    default_val if default_val in q['options'] else q['options'][0]
                )
            else:
    ...
    if st.session_state.finished:
        ...
        if q['type'] == 'slider':
            st.slider(
                label=q['label'], min_value=q['min'], max_value=q['max'],
                step=q['step'], format=q.get('format'),
                key=wkey, label_visibility='hidden',
                on_change=save_answer, args=(q['key'], wkey),
            )
            if 'min_label' in q:
                cl, cr = st.columns(2)
            ...
            st.radio(
                label=q['label'], options=q['options'],
